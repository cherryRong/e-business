<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:62:"C:\phpStudy\WWW\tp50\public/../app/index\view\goods\index.html";i:1542683279;}*/ ?>
...
<div class="i_bg">
    ...
    <div class="content">
        ...
        <div id="tsShopContainer">
            <div id="tsImgS">
                <a href="/uploads/<?php echo $ginfo['goods_img']; ?>" title="Images" class="MagicZoom" id="MagicZoom">
                    <img src="/uploads/<?php echo $ginfo['goods_img']; ?>" width="390" height="390" />
                </a>
            </div>
                    ...
                    <ul>
                       <?php foreach($gallery as $val): ?>
                        <li onclick="showPic(0)" rel="MagicZoom" class="tsSelectImg">
                          <img src="/uploads/<?php echo $val['original_img']; ?>" tsImgS="/uploads/<?php echo $val['original_img']; ?>" width="79" height="79" />
                        </li>
                      <?php endforeach; ?> 
        ...
        <div class="pro_des">
        	<div class="des_name">
            	<p><?php echo $ginfo['goods_name']; ?></p>
                <?php echo $ginfo['goods_brief']; ?>
            </div>
            <div class="des_price">
            	本店价格：<b>￥<?php echo $ginfo['shop_price']; ?></b><br />
                库存：<span><?php echo $ginfo['goods_number']; ?></span>
            </div>
            <?php foreach($attr as $val): ?>
            <div class="des_choice">
            	<span class="fl"><?php echo $val['attr_name']; ?>：</span>
                <ul>
                   <?php foreach($val['attr_value'] as  $k => $v): ?>
                	<li class="" goods_attr_id = <?php echo $k; ?> ><?php echo $k; ?> : <?php echo $v; ?><div class="ch_img"></div></li>
                   <?php endforeach; ?>
            ...
            <div class="des_join">
                ...
                <span class="fl"><a class="cart"><img src="images/j_car.png"  onclick="addCart()" /></a></span>
            </div>            
        </div>    
        
        <div class="s_brand">
        	<div class="s_brand_img"><img src="/uploads/<?php echo $binfo['brand_logo']; ?>" width="188" height="132" /></div>
            <div class="s_brand_c"><a href="<?php echo url('Brand/index'); ?>">进入<?php echo $binfo['brand_name']; ?>品牌专区</a></div>
        </div>    
            ...
            <div class="des_border">
                ...
                <div class="des_con" id="p_attribute">
                	...
                	<table border="0" align="center" style="width:100%; font-family:'宋体'; margin:10px auto;" cellspacing="0" cellpadding="0">
                      <tr>
                       <?php foreach($down as $val): ?>
                          <td><?php echo $val['attr_name']; ?> ：<?php echo $val['attr_value']; ?></td>
                       <?php endforeach; ?>
                      </tr>
    ...
    <div id="MyLogin" class="white_content">             
        <div class="white_d">
            <div class="notice_t">
                <span class="fr" style="margin-top:10px; cursor:pointer;" onclick="CloseDiv('MyLogin','fade')"><img src="images/close.gif" /></span>
            </div>
            <div class="notice_c">
                ...
                <table border="0" align="center" style="margin-top:;" cellspacing="0" cellpadding="0">
                  <tr valign="top">
                    <td width="40"><img src="images/suc.png" /></td>
                    <td>
                        <span style="color:#3e3e3e; font-size:18px; font-weight:bold;">登录</span><br />
                    </td>
                  </tr>
                <tr>
                    <td>账号：</td>
            ...
            <div class="notice_c">
                ...
                <table border="0" align="center" style="margin-top:;" cellspacing="0" cellpadding="0">
                  <tr valign="top">
                    <td width="40"><img src="images/suc.png" /></td>
                    <td>
                    	<span style="color:#3e3e3e; font-size:18px; font-weight:bold;" id="cstatus">宝贝已成功添加到购物车</span><br />
                    	<span id="cinfo"> 购物车共有1种宝贝（3件） &nbsp; &nbsp; 合计：1120元 </span>
                    </td>
                  </tr>
                  <tr height="50" valign="bottom">
                  	<td>&nbsp;</td>
                    <td><a href="<?php echo url('Cart/index'); ?>" class="b_sure">去购物车结算</a><a href="#" class="b_buy">继续购物</a></td>
                  </tr>
                </table>
...
<script>
    $(function(){
        //全局变量数组
        var attrId = Array();
        //onclick="ShowDiv_1('MyDiv1','fade1')"  弹出购物车的框
        $(".des_choice ul li").click(function(){
            //拿当前对象
            _this = $(this);
            //有无class属性值为checkd/  没有加上  /有则移除
        ...
        $("#login").click(function(){
            var uname = $("#uname").val();
            var pass = $("#pass").val();
            var url = "<?php echo url('Cart/login'); ?>";
            $.post(url,{'uname':uname,'pass':pass},function(msg){
                 if(msg==1){
                    alert("登录成功");
                    CloseDiv('MyLogin','fade');
                    addCart();
                 }else{
                    alert("登录失败");
                 }
        ...
        function getAttr(){
            //拿到所有的li标签
            var li = $(".des_choice ul li");
            //清空原先的数据
            attrId = Array();
            //循环
            li.each(function(k,v){
               //判断是否选中
               if(li.eq(k).attr("class")=="checked"){
                  attrId.push(li.eq(k).attr("goods_attr_id"));
       ...
       function checkAttr(){
          //规格的长度
          var len = $(".des_choice").length;
           if(len == attrId.length){
             return true;
           }else{
             return false;
           }
       ...
       //加入购物车
        function addCart(){
           ...
           var goods_attr_id = attrId.join();
           var url = "<?php echo url('Cart/add'); ?>";
           $.get(url,{'goods_id':goods_id,"buy_number":buy_number,"goods_attr_id":goods_attr_id},function(msg){
                 //判断是否登录
                 if(msg.flag == 0){
                   
                    //没登陆  弹出登录框
                    ShowDiv('MyLogin','fade');
                 }else{
                      //换cstatus
                     $("#cstatus").html(msg.message)
                     var str = '购物车共有'+msg.cinfo.num+'种宝贝（'+msg.cinfo.sumNum+'件） &nbsp; &nbsp; 合计：'+msg.cinfo.money+'元';
                     //cinfo
                     $("#cinfo").html(str);
                     //弹出购物车的框
